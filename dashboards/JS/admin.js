...
let adminEmployeesShellPromise = null;

async function loadAdminEmployeesShell() {
    if (!adminEmployeesShellPromise) {
        adminEmployeesShellPromise = fetch('templates/admin_employees_shell.html').then(resp => {
            if (!resp.ok) throw new Error('Failed to load admin employees shell');
            return resp.text();
        });
    }
    return adminEmployeesShellPromise;
}
...
async function renderEmployees() {
    ...
    main.innerHTML = await loadAdminEmployeesShell();
